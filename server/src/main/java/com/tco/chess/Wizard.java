...
public class Wizard extends ChessPiece {
	...
	public ArrayList<String> legalMoves() {
		...
		for (int row = 0; row < this.board.getBoardSize(); row++) {
			for (int col = 0; col < this.board.getBoardSize(); col++) {
				...
				if (isValidWizardMove(moveX, moveY) && !isTeamPiece(toIndices)) {
    ...
    private boolean isValidWizardMove(int moveX, int moveY) {
        if ((Math.abs(moveX) == 1 && Math.abs(moveY) == 3) || 
            (Math.abs(moveX) == 3 && Math.abs(moveY) == 1) ||
